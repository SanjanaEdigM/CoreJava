...
public class Hospital {
   ...
   public PatientDto getPatientByName(String patientName) {
	   ...
	   public PatientDto getPatientByContactNo(long patientContactNo) throws Exception{
		   PatientDto dto1=null;
		   System.out.println("Inside getPatientName");
	    ...
	    public PatientDto getPatientByGender(char patientGender) throws Exception {
		   PatientDto dto1=null;
		   System.out.println("Inside getPatientName");
		    if(patientGender!='\0'){
				for(int i=0;i<patient.length;i++)
	   ...
	   public PatientDto getPatientByAge(int patientAge) throws Exception{
		   PatientDto dto1=null;
		   System.out.println("Inside getPatientName");
		    if(patientAge!=0){
				for(int i=0;i<patient.length;i++)
	   ...
	   public PatientDto getPatientByBloodGroup(String patientBloodGroup) throws Exception{
		   PatientDto dto1=null;
		   System.out.println("Inside getPatientName");
		   ...
		   if(areaName!=null) {
			   for(int i=0;i<patient.length;i++) {
				   ...
				   if(patient[i].getAddress().getCountry().getState().getDistrict().getArea().getAreaName().equals(areaName)) {
					   System.out.println("Patient found by areaName "+areaName ); 
					   patient1=patient[i];
				   }
				   else {
	   ...
	   public PatientDto getPatientByAreaId(int areaId) {
		  PatientDto patient1=null;
		  System.out.println("Inside getPatientByAreaId");
		  if(areaId!=0) {
			  for(int i=0;i<patient.length;i++) {
				  System.out.println(patient[i]);
				  
				  if(patient[i].getAddress().getCountry().getState().getDistrict().getArea().getAreaId()==areaId) {
					  System.out.println("Patient found by areaId"+areaId);
					  patient1=patient[i];
				  }
				  else {
					  System.out.println("AreaId not found");
				  }
			  }
		  }
